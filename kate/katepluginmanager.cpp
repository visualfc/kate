...
void KatePluginManager::enablePluginGUI(KatePluginInfo *item, KateMainWindow *win, KConfigBase *config)
{
    ...
    if (!win->pluginViews().contains(item->plugin)) {
        ...
        if (createdView) {
            ...
            auto client = dynamic_cast<KXMLGUIClient *>(createdView);
            if (client) {
                win->guiFactory()->addClient(client);
            }
        }
    }
...
void KatePluginManager::disablePluginGUI(KatePluginInfo *item, KateMainWindow *win)
{
    ...
    auto client = dynamic_cast<KXMLGUIClient *>(deletedView);
    if (client) {
        win->guiFactory()->removeClient(client);
    }
    delete deletedView;
